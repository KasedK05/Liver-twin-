...
class ProteinHomeostasis(Subsystem):
    def initialize_state(self, params):
        return {
            "translation": params.get("trans_0", 0.60),
            "upr": params.get("upr_0", 0.05),
            "aggregates": params.get("agg_0", 0.05),
            "secretory_load": params.get("sec_0", 0.40),
        }

    def update(self, dt, c, d):
        s = self.state
        trans_capacity = 0.5 * c.get("atp", 0.5)
        s["translation"] += dt * (0.05 * (trans_capacity - s["translation"]) - d.get("trans_inhibit", 0.0))
        s["translation"] = np.clip(s["translation"], 0.0, 1.0)

        upr_trigger = 1.0 if (s["aggregates"] > 0.4 or d.get("proteasome_inhibit", 0) > 0.5) else 0.0
        ...
        s["secretory_load"] += dt * (sec_load - sec_capacity)
        s["secretory_load"] = np.clip(s["secretory_load"], 0.0, 1.0)

    def get_health_score(self):
        s = self.state
        return 0.4 * s["translation"] + 0.3 * (1 - s["upr"]) + 0.3 * (1 - s["aggregates"])
