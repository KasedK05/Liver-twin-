...
class SignalingControl(Subsystem):
    def initialize_state(self, params):
        return {"mtor": params.get("mtor_0", 0.50), "p53": params.get("p53_0", 0.10), "growth_factor": params.get("gf_0", 0.40), "cytokine": params.get("cyto_0", 0.10)}
    ...
    def update(self, dt, c, d):
        ...
        gf_input = d.get("growth_factor", 0.0)
        gf_decay = 0.05 * s["growth_factor"]
        s["growth_factor"] += dt * (gf_input - gf_decay)
        s["growth_factor"] = np.clip(s["growth_factor"], 0.0, 1.0)
        cyto_drive = 0.3 * c.get("inflammasome_activity", 0) + 0.2 * c.get("viral_load", 0)
        cyto_suppress = d.get("cytokine_suppress", 0.0)
        s["cytokine"] += dt * (0.05 * cyto_drive - 0.03 * s["cytokine"] - cyto_suppress)
        s["cytokine"] = np.clip(s["cytokine"], 0.0, 1.0)

    def get_health_score(self):
        s = self.state
        return 0.3 * s["mtor"] + 0.3 * (1 - s["p53"]) + 0.2 * s["growth_factor"] + 0.2 * (1 - s["cytokine"])
